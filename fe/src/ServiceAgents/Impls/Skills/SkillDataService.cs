...
namespace employee.skill.fe.ServiceAgents.Impls.Skills
{
  public class SkillDataService : ISkillDataService
  {
    ...
    public async Task<int> FetchAvailableSkillsCount(string authorizationToken = null)
    {
      int result = 0;

      try
      {
        var client = new RestClient($"{BaseAddr}/api/Skills/count-available");
        var request = new RestRequest(Method.GET);
    ...
    public async Task<int> FetchSkillsInUseCount(string authorizationToken = null)
    {
      int result = 0;

      try
      {
        var client = new RestClient($"{BaseAddr}/api/Skills/count-use");
        var request = new RestRequest(Method.GET);
